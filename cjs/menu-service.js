...
function menuRows(result) {
    if (!result.success || result.rowsTruncated)
        throw new Error(result.error || 'Menu query failed or was truncated.');
    const data = typeof result.output === 'string' ? JSON.parse(result.output) : result.output;
    if (!data || !Array.isArray(data.Tables) || data.Tables.length !== 1 || !Array.isArray(data.Tables[0].Rows))
        throw new Error('Unexpected native menu dataset.');
    return data.Tables[0].Rows;
}
const same = (a, b) => String(a ?? '').toLowerCase() === String(b ?? '').toLowerCase();
const stable = (rows) => JSON.stringify(rows.map(r => Object.fromEntries(Object.entries(r).sort())).sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b))));
class MenuMcpService {
    service;
    plans = new Map();
    constructor(service) {
        this.service = service;
    }
    session() { return this.service.getSessionKey(); }
    ...
    async configuration(group, item) {
    ...
    async form(input) {
        ...
        if (typeof ids === 'string') {
            try {
                ...
                throw new Error('Menu backend form resolution failed: ' + ids);
            }
        }
        if (!resolved.success || !Array.isArray(ids) || ids.length !== 1 || !Array.isArray(ids[0]) || ids[0].length !== 2)
            throw new Error('Cannot resolve authoritative application and form IDs. Install SCM_API.MenuManagement.');
        const [appId, nativeFormId] = ids[0];
        ...
        if (!formId || !same(formId, nativeFormId))
            throw new Error('Cannot resolve runtime GUID from Form XML.');
        return { appId, formId, commandName: parts[3] + '.' + parts[6] };
    }
    async execute(tool, args) {
        if (tool === 'get_menu_configuration') {
            const input = menu_schema_js_1.menuSchemas.get_menu_configuration.parse(args);
        ...
        if (tool === 'plan_menu_item') {
            const input = menu_schema_js_1.menuSchemas.plan_menu_item.parse(args);
            const languages = await this.service.getLanguages();
            if (Object.keys(input.captions).some(lang => !languages.includes(lang)))
                throw new Error('Unknown menu caption language.');
            ...
            if (groups.length !== 1)
                throw new Error('Choose one existing visible HTML menu group.');
            input.group = String(groups[0].NAME);
            if (cfg.items.some(r => r.PARENT && same(r.NAME, input.itemName)))
                throw new Error('Menu item already exists; this tool creates only and will not overwrite it.');
            const roles = input.roles.map(name => {
                const matches = cfg.roles.filter(r => same(r.ROLE, name));
                if (matches.length !== 1 || !matches[0].ROLEID || ['*', '@@'].includes(String(matches[0].ROLEID)))
                    throw new Error('Unknown, ambiguous or special role: ' + name);
                return matches[0];
            });
            if (new Set(roles.map(r => r.ROLEID)).size !== roles.length)
                throw new Error('Duplicate roles.');
            ...
            if (input.position <= maxPosition)
                throw new Error('First version appends items; choose a position after existing items.');
            ...
            if (this.plans.size >= 100)
                throw new Error('Too many pending menu plans.');
        ...
        const { planId } = menu_schema_js_1.menuSchemas.apply_menu_item.parse(args);
        const plan = this.plans.get(planId);
        if (!plan || plan.session !== this.session() || plan.expires < Date.now())
            throw new Error('Menu plan expired or session changed. Plan again.');
        if (plan.state === 'done')
            return plan.result;
        if (plan.state !== 'ready')
            throw new Error('Plan already attempted. Inspect configuration before creating another plan.');
        plan.state = 'applying';
        const completed = [];
        ...
        try {
            ...
            const cfg = await this.configuration(input.group, input.itemName);
            if (stable([...cfg.items, ...cfg.grants, ...cfg.captions]) !== plan.baseline)
                throw new Error('Menu changed since planning.');
            const form = await this.form(input);
            if (form.appId !== plan.appId || form.formId !== plan.formId || plan.roles.some(role => !cfg.roles.some(r => same(r.ROLE, role.ROLE) && r.ROLEID === role.ROLEID)))
                throw new Error('Form or role mapping changed.');
            ...
            if (!result.success || ![true, 'true', '.T.'].includes(result.output))
                throw new Error('Owned menu backend did not confirm success: ' + JSON.stringify(result.error || result.output));
...
exports.MenuMcpService = MenuMcpService;
