...
exports.ensureFormResourceBinding = ensureFormResourceBinding;
exports.inspectFormResourceBinding = inspectFormResourceBinding;
const xmldom_1 = require("@xmldom/xmldom");
const child = (element, name) => Array.from(element.childNodes)
    .find((node) => node.nodeType === 1 && node.localName === name);
/** Match the native Enterprise_Designer.XFD2HTMLResourcesCopy ResTag3 contract. */
function ensureFormResourceBinding(formXml, formId, language) {
    ...
    const document = new xmldom_1.DOMParser({ onError: (_level, message) => { throw new Error(message); } }).parseFromString(formXml, 'application/xml');
    ...
    if (!existing) {
        const marker = append(resources, 'FromDesigner');
    ...
    return { xml: changed ? new xmldom_1.XMLSerializer().serializeToString(document) : formXml, changed };
}
/** Read-only structural diagnostics; this is not a Designer or runtime validation. */
function inspectFormResourceBinding(formXml, formId, language) {
    const document = new xmldom_1.DOMParser({ onError: (_level, message) => { throw new Error(message); } }).parseFromString(formXml, 'application/xml');
    const root = document.documentElement;
    const warnings = [];
    ...
    if (embeddedGuid.toLowerCase() !== formId.toLowerCase())
        warnings.push('Form XML Guid differs from the enterprise GUID. The enterprise GUID is used for Resources binding.');
    const missingColumnTypes = [];
    for (const element of Array.from(document.getElementsByTagName('*'))) {
        if (element.localName !== '__array__Columns')
            continue;
    ...
    if (missingColumnTypes.length)
        warnings.push('Column definitions are missing xtype. Designer may deserialize them as plain Objects. Compare against a Designer-generated form of the same control type.');
    let status;
    try {
        status = ensureFormResourceBinding(formXml, formId, language).changed ? 'repair_required' : 'valid';
    }
    catch (error) {
        status = 'unsupported';
...
//# sourceMappingURL=form-resource-binding.js.map