...
const table_definition_js_1 = require("../table-definition.js");
const checkin_verification_js_1 = require("../checkin-verification.js");
const xmldom_1 = require("@xmldom/xmldom");
const form_resources_js_1 = require("../form-resources.js");
const form_resource_binding_js_1 = require("../form-resource-binding.js");
...
class StarlimsHttpAdapter {
    ...
    constructor(config, logger, fetchImpl = fetch) {
        ...
        this.capabilities = Object.freeze(config.permissionPolicy === 'allow-writes'
            ...
            : [...READ_CAPABILITIES]);
        this.menus = new menu_service_js_1.MenuMcpService({
    ...
    async invoke(tool, arguments_) {
        const contract = (0, catalog_js_1.findToolContract)(tool);
        if (!contract)
        ...
        switch (tool) {
            ...
            case 'execute_database_change': {
                this.assertWriteAllowed(tool);
                return (0, query_database_js_1.databaseQueryResult)(await this.request('McpExecuteDatabaseChange', { method: 'POST', body: (0, query_database_js_1.prepareDatabaseChange)(arguments_) }));
            }
            case 'query_database': return (0, query_database_js_1.databaseQueryResult)(await this.request('McpQueryDatabase', { method: 'POST', body: (0, query_database_js_1.prepareDatabaseQuery)(arguments_) }));
    ...
    async listCheckedOutItems(args) {
        ...
        (0, checkin_verification_js_1.pendingCheckoutIds)(response.data); // Reject unavailable status instead of reporting an empty list.
        let items;
        if (typeof response.data === 'string') {
            const doc = new xmldom_1.DOMParser({ onError: (_level, message) => { throw new Error(message); } }).parseFromString(response.data, 'application/xml');
    ...
    async readTableXml(uri) {
        ...
        (0, table_definition_js_1.tableDefinitionVersion)(response.data);
        return response.data;
    }
    async checkoutStatus() {
        const response = await this.request('GetCheckedOutItems');
        this.assertAccepted(response, 'Read checkouts');
        return (0, checkin_verification_js_1.pendingCheckoutIds)(response.data);
    }
    async checkoutTable(args) {
        const uri = this.tableUri(String(args.uri)), id = (0, table_definition_js_1.tableDefinitionId)(await this.readTableXml(uri));
        if (!id)
    ...
    async createTable(args) {
        ...
        return { uri, created: true, tableName, dsn, verified: true, version: (0, form_resources_js_1.contentVersion)((0, table_definition_js_1.tableDefinitionVersion)(definition)), result: response.data };
    }
    async editTable(args) {
        const uri = this.tableUri(String(args.uri)), requested = (0, table_definition_js_1.prepareTableCaptionXml)(String(args.tableXml));
        const before = await this.readTableXml(uri), id = (0, table_definition_js_1.tableDefinitionId)(before);
        if (!id || (0, table_definition_js_1.tableDefinitionId)(requested).toLowerCase() !== id.toLowerCase())
            throw new Error('The submitted TableDTO must retain the target table Id.');
        const version = (xml) => (0, form_resources_js_1.contentVersion)((0, table_definition_js_1.tableDefinitionVersion)(xml));
        ...
        const definition = await (0, table_definition_js_1.waitForTableReadBack)(() => this.readTableXml(uri), requested, before);
        return { uri, saved: true, verified: true, definition, version: version(definition), result: response.data };
    }
    async undoCheckout(args) {
        const uri = (0, checkin_verification_js_1.checkinTargetUri)(String(args.uri)), guid = await this.resolveTargetId(uri);
    ...
    async resolveTargetId(uri) {
        if (uri.startsWith('/Tables/')) {
            const id = (0, table_definition_js_1.tableDefinitionId)(await this.readTableXml(this.tableUri(uri)));
            if (id)
                return id;
        }
        else {
            ...
            const guid = items.find(item => (0, checkin_verification_js_1.checkinTargetUri)(String(item.uri || item.id)).toLowerCase() === uri.toLowerCase())?.guid;
    ...
    async getTableDefinition(args) {
        ...
        return { uri, definition: bounded.value, version: (0, form_resources_js_1.contentVersion)((0, table_definition_js_1.tableDefinitionVersion)(definition)), totalCharacters: bounded.totalCharacters, truncated: bounded.truncated };
    }
    ...
    async checkout(args) {
        ...
        if (/\/(HTMLForms|XFDForms)\//i.test(uri)) {
            const targetUri = (0, checkin_verification_js_1.checkinTargetUri)(uri);
            ...
            const guid = items.find(item => (0, checkin_verification_js_1.checkinTargetUri)(String(item.uri || item.id)).toLowerCase() === targetUri.toLowerCase())?.guid;
            ...
            const pending = (0, checkin_verification_js_1.pendingCheckoutIds)(status.data);
    ...
    async checkin(args) {
        ...
        (0, checkin_verification_js_1.assertCheckinAccepted)(response);
        if (hasTarget(await status()))
    ...
    async saveFormResources(args) {
        ...
        const xml = (0, form_resources_js_1.toProgrammaticFormResources)(String(args.resourceXml || ''), current.xml);
    ...
    async inspectHtmlFormResources(resourceUri, language) {
        ...
        try {
            ...
            return { uri, checkoutLanguage, writableInRequestedLanguage: checkoutLanguage ? checkoutLanguage === language : null, ...(0, form_resource_binding_js_1.inspectFormResourceBinding)((await this.readCode(uri, language)).code, formId, language) };
        }
        catch (error) {
    ...
    async prepareHtmlFormResourceBinding(resourceUri, language) {
        ...
        const before = (await this.readCode(uri, language)).code;
        return { uri, formId, language, before, ...(0, form_resource_binding_js_1.ensureFormResourceBinding)(before, formId, language) };
    }
    async saveHtmlFormResourceBinding(binding) {
        if (!binding)
            return { formBindingVerified: false, formBindingUpdated: false };
        if (binding.changed)
            await this.saveVerified(binding.uri, binding.language, binding.xml, (0, form_resources_js_1.contentVersion)(binding.before));
        const actual = (await this.readCode(binding.uri, binding.language)).code;
        if ((0, form_resource_binding_js_1.ensureFormResourceBinding)(actual, binding.formId, binding.language).changed) {
