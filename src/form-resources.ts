...
function resourceFormat(document: Document): 'programmatic' | 'designer' {
  if (!document.documentElement) throw new Error('Missing Form Resources root element.');
...
function parseDocument(xml: string): Document & { documentElement: Element } {
...
function serializeProgrammaticResources(resources: FormResourceEntry[]): string {
  ...
  if (!root) throw new Error('Could not create ResourcesDataset.');
  for (const entry of resources) {
