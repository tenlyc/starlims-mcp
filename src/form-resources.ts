...
function decodeXmlEntity(value: string): string {
  return value.replace(/&#(x?[0-9a-f]+);|&(lt|gt|amp|quot|apos);/gi, (match, numeric: string | undefined, named: string | undefined) => {
    if (numeric) {
    ...
    return ({ lt: '<', gt: '>', amp: '&', quot: '"', apos: "'" } as Record<string, string>)[String(named).toLowerCase()] || match;
  });
}
...
type XmlNode = { name: string; start: number; openEnd: number; closeStart: number; end: number; children: XmlNode[] };
...
function xmlNodes(xml: string): XmlNode[] {
  ...
  for (const match of xml.matchAll(tags)) {
    ...
    if (token.startsWith('</')) {
      ...
      if (!node || node.name.toLowerCase() !== name.toLowerCase()) throw new Error(`Invalid Form Resources XML near closing tag ${name}.`);
      node.closeStart = start;
      node.end = start + token.length;
      continue;
    }
    const node: XmlNode = { name, start, openEnd: start + token.length, closeStart: start + token.length, end: start + token.length, children: [] };
    const parent = stack[stack.length - 1];
    if (parent) parent.children.push(node); else roots.push(node);
    if (!token.endsWith('/>')) stack.push(node);
  }
  if (stack.length) throw new Error(`Invalid Form Resources XML: unclosed tag ${stack[stack.length - 1].name}.`);
  return roots;
}

function nodeText(xml: string, node: XmlNode | undefined): string | undefined {
  return node ? decodeXmlEntity(xml.slice(node.openEnd, node.closeStart)) : undefined;
}

function resourceRows(xml: string): Array<{ full: string; tag: string; start: number; end: number; entry: FormResourceEntry }> {
  const rows: Array<{ full: string; tag: string; start: number; end: number; entry: FormResourceEntry }> = [];
  const visit = (node: XmlNode): void => {
    const child = (name: string) => node.children.find((candidate) => candidate.name.toLowerCase() === name.toLowerCase());
...
export function normalizeFormResourcesUri(uri: string): string {
  ...
  if (!match) throw new Error('Form Resources URI must identify an HTML/XFD form XML, CodeBehind, Guide, or Resources document.');
  return `${match[1]}/Resources/${match[2]}`;
}

export function decodeFormResourcePayload(payload: string): string {
  const trimmed = payload.trim();
  if (!trimmed || (XML_PREFIX.test(trimmed) && trimmed.includes('<'))) return payload;
  try {
...
export function parseFormResources(payload: string): { xml: string; resources: FormResourceEntry[] } {
  const xml = decodeFormResourcePayload(payload);
  if (!xml.trim().startsWith('<') || !xml.trim().endsWith('>')) throw new Error('Invalid Form Resources XML.');
...
export function setFormResourceValue(payload: string, resourceId: string, resourceValue: string): { xml: string; created: boolean } {
  ...
  if (!rootClose) throw new Error('Unable to locate the Form Resources root element.');
  const index = rootClose.index as number;
