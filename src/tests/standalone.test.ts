...
test('shared server alone registers all 39 DevTools tools and preserves screenshot results', async context => {
  ...
  assert.equal(listed.length, 39); assert.equal(new Set(listed).size, 39);
...
async function startMockScmApi(options: { checkinMode?: 'error-string' | 'retain' | 'invalid-status'; checkoutLanguage?: string; beforeReadCode?: (uri: string, count: number, codes: Map<string, string>) => void } = {}): Promise<{ baseUrl: string; close(): Promise<void>; codes: Map<string, string>; requests: string[] }> {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (endpoint === 'TableGetById') return json(response, { success: true, data: '<TableDTO><Id>table-guid</Id><Name>BATCHES</Name></TableDTO>' });
    if (endpoint === 'GetCode') {
      const uri = url.searchParams.get('URI') || '';
      if (uri.startsWith('/ServerLogs/')) return json(response, { success: true, data: { code: 'first\nlast', language: 'LOG' } });
...
test('CLI exposes the standalone read-only server over stdio', async (context) => {
  ...
  assert.equal((log.structuredContent as { log: string }).log, 'last');
