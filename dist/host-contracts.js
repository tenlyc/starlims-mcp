...
const localDefinitions = [...Object.entries(menuSchemas).map(([id, inputSchema]) => ({
        id, title: id, description: { get_menu_configuration: 'Read native HTML menu configuration and role IDs.', plan_menu_item: 'Prepare a create-only menu plan for an existing group. No writes. ' + MENU_WORKFLOW_INSTRUCTIONS, apply_menu_item: 'Apply a user-confirmed menu plan and verify configuration. Grants menu access to specified roles. Uses the separate SCM_API.MenuManagement transaction; does not modify system designer methods. Runtime acceptance is separate.' }[id],
...
export const HOST_TOOL_CONTRACTS = [
