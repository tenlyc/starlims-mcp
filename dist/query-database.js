...
export function prepareDatabaseQuery(input) {
    ...
    if (words.some(word => QUERY_FORBIDDEN.includes(word)))
    ...
    for (const match of sql.matchAll(/([A-Za-z_][A-Za-z0-9_]*)\s*\(/g)) {
        ...
        if (prefix.endsWith('.') || !QUERY_CALLS.includes(match[1].toUpperCase()))
...
export function databaseQueryResult(payload) {
    const value = payload;
...
export function prepareDatabaseChange(input) {
...
export function databaseChangeConfirmation(change) {
