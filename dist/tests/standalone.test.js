...
test('shared server alone registers all 39 DevTools tools and preserves screenshot results', async (context) => {
    ...
    assert.equal('imageData' in screenshot.structuredContent, false);
...
async function startMockScmApi(options = {}) {
    const codes = new Map([[`${codeUri}|ENG`, 'return "hello";'], [`${resourcesUri}|CHS`, initialResources], [`${formUri}|CHS`, initialForm]]);
    ...
    const server = createServer(async (request, response) => {
        ...
        if (endpoint === 'GetSessions')
            return json(response, { success: true, data: { starlimssessionid: 's1', langid: 'ENG' } });
        if (endpoint === 'Version')
            return json(response, { success: true, data: '1.8.2' });
        if (endpoint === 'GetEnterpriseItems')
            return json(response, { success: true, data: { items: [{ name: 'Hello', uri: codeUri, type: 'SS' }] } });
        if (endpoint === 'Search')
            return json(response, { success: true, data: { items: url.searchParams.get('itemName') === 'Equipment_Ledger' ? [{ name: 'Equipment_Ledger', uri: formUri, guid: formGuid }] : [{ name: 'Hello', uri: codeUri, guid: 'code-guid' }] } });
        if (endpoint === 'GlobalSearch')
            return json(response, { success: true, data: { items: [{ name: 'Hello', uri: codeUri, guid: 'code-guid' }], totalCount: 1 } });
        if (endpoint === 'GetCheckedOutItems')
            return json(response, { success: true, data: options.checkinMode === 'invalid-status' && checkinCalled ? 'invalid' : `<DataSet>${[...pending].map((id) => `<PendingCheckins><CHILDID>${id}</CHILDID><LANGID>${options.checkoutLanguage || 'CHS'}</LANGID></PendingCheckins>`).join('')}</DataSet>` });
        ...
        if (endpoint === 'CheckIn') {
            ...
            if (options.checkinMode === 'error-string')
                return json(response, { success: true, data: 'ERROR: wrong user' });
            if (options.checkinMode !== 'retain')
                pending.delete(url.searchParams.get('URI') === codeUri ? 'code-guid' : formGuid);
    ...
    if (!address || typeof address === 'string')
        throw new Error('Mock SCM_API did not start.');
    return {
...
test('designer merge blocks a concurrent resource update without a caller version', async (context) => {
    ...
    const mock = await startMockScmApi({ beforeReadCode: (uri, count, codes) => {
            if (uri === resourcesUri && count === 2)
                codes.set(`${uri}|CHS`, concurrent);
        } });
...
test('standalone adapter reads, searches and retrieves multilingual resources from a mock SCM_API', async (context) => {
    ...
    const browse = await adapter.invoke('browse_tree', {});
    const search = await adapter.invoke('search_by_name', { query: 'Hello' });
    const table = await adapter.invoke('get_table_definition', { uri: '/Tables/BATCHES' });
    const resources = await adapter.invoke('get_form_resources', { uri: resourcesUri, language: 'CHS', includeXml: true });
...
test('write policy enforces versions and verifies save, checkout, resources and check-in against a mock SCM_API', async (context) => {
    ...
    await adapter.connect();
    const read = await adapter.invoke('get_item_code', { uri: codeUri, language: 'ENG' });
    const family = await adapter.invoke('checkout_item', { uri: resourcesUri, language: 'CHS' });
    ...
    const checkout = await adapter.invoke('checkout_item', { uri: codeUri, language: 'ENG' });
    const saved = await adapter.invoke('save_item', { uri: codeUri, language: 'ENG', code: 'return "changed";', expectedVersion: read.version });
    ...
    const resourceRead = await adapter.invoke('get_form_resources', { uri: resourcesUri, language: 'CHS' });
    const resourceSaved = await adapter.invoke('set_form_resource', { uri: resourcesUri, language: 'CHS', resourceId: 'TITLE', resourceValue: '设备台账', expectedVersion: resourceRead.version });
    ...
    const checkedIn = await adapter.invoke('checkin_item', { uri: codeUri, language: 'ENG', reason: 'integration test' });
...
test('streamable HTTP transport serves the same shared MCP runtime with bearer protection', async (context) => {
    ...
    const version = initial.structuredContent.version;
...
test('resource writes reject a mismatched checkout language before SaveCode', async () => {
    ...
    try {
        ...
        const read = await adapter.invoke('get_form_resources', { uri: resourcesUri, language: 'CHS' });
...
for (const mode of ['error-string', 'retain', 'invalid-status']) {
...
test('checkin verifies parent form checkout release for a Resources URI', async () => {
    ...
    try {
        const config = await loadStarlimsMcpConfig([], { STARLIMS_BASE_URL: mock.baseUrl, STARLIMS_USER: 'developer', STARLIMS_PASSWORD: 'secret-pass', STARLIMS_MCP_PERMISSION_POLICY: 'allow-writes' });
        const adapter = new StarlimsHttpAdapter(config, logger);
        const result = await adapter.invoke('checkin_item', { uri: resourcesUri, language: 'CHS', reason: 'test' });
