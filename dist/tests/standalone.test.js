...
async function startMockScmApi() {
    const codes = new Map([[`${codeUri}|ENG`, 'return "hello";'], [`${resourcesUri}|CHS`, initialResources]]);
    const requests = [];
    const server = createServer(async (request, response) => {
        ...
        if (endpoint === 'Search')
            return json(response, { success: true, data: { items: [{ name: url.searchParams.get('itemName'), uri: codeUri }] } });
        if (endpoint === 'GlobalSearch')
            return json(response, { success: true, data: { items: [{ name: 'Hello', uri: codeUri }], totalCount: 1 } });
        ...
        if (endpoint === 'CheckOut' || endpoint === 'CheckIn')
            return json(response, { success: true, data: true });
