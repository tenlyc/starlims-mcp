...
function fixture(policy = 'allow-writes') {
    const calls = [];
    const pending = new Set();
    const tree = [{ NAME: 'Demo', PARENT: null }];
    const roles = [{ ROLE: 'Lab_Admin', ROLEID: 'role-id' }];
    const grants = [], captions = [];
    const state = { definition, log: 'old\r\nwarning\r\nerror\r\n', menuWrites: 0, rejectCreate: false,
        ...
        override: undefined };
    const config = { baseUrl: 'https://example.test/LIMS', user: 'engineer', password: 'test-secret',
    ...
    const adapter = new StarlimsHttpAdapter(config, logger, (async (input, options) => {
        ...
        const override = state.override?.(call);
        if (override)
            return override;
        ...
        switch (endpoint) {
            ...
            case 'GetCheckedOutItems':
                if (state.staleOnStatus)
                    state.definition = state.definition.replace('<Length>10</Length>', '<Length>12</Length>');
            ...
            case 'CheckOut':
                pending.add('table-id');
                return ok(null);
            case 'CheckIn':
                if (!state.retainCheckin)
            ...
            case 'TableSave':
                if (!state.saveNoop)
                    state.definition = body.TableXml;
                return ok('saved');
            case 'Add': return ok({ success: !state.rejectCreate, data: 'creation rejected' });
            case 'RunScript': {
                if (body.URI === '/ServerScripts/SCM_API/McpGetCheckInHistory')
                    return ok({ success: true, items: [{ uri: scriptUri, checkedInBy: body.Parameters[0] }] });
                if (body.URI === '/ServerScripts/SCM_API/MenuManagement') {
                    if (body.EntryPoint === 'ResolveForm')
                        return ok([['app-id', formId]]);
                    state.menuWrites++;
                    if (state.failMenu)
                    ...
                    for (const [lang, caption] of p[8])
                        captions.push({ LANGID: lang, CAPTION: caption });
                    for (const role of p[9])
                        grants.push({ PARENT: p[0], NAME: p[1], ROLEID: role });
                    return ok(true);
                }
...
test('standalone exposes all server-side contracts and no desktop dependencies', async () => {
    ...
    f.config.user = 'other';
    assert.equal(f.adapter.config.user, 'engineer');
});
test('logs honor user and tail limits, handle legacy text, and distinguish failure from empty', async () => {
    ...
    const log = await f.adapter.invoke('read_log', { user: 'tester', maxLines: 2 });
    ...
    for (const user of ['../x', 'x/y', 'x\\y', '\u0000'])
        await assert.rejects(f.adapter.invoke('read_log', { user }), /Invalid/);
    f.state.log = 'There is no log file for engineer';
    assert.equal((await f.adapter.invoke('read_log', {})).empty, true);
    f.state.log = 'ERROR: real server failure';
    assert.equal((await f.adapter.invoke('read_log', {})).log, f.state.log);
    for (const response of [() => new Response('legacy\nlast', { headers: { 'content-type': 'text/plain' } }), () => json('legacy\nlast')]) {
        ...
        assert.equal((await f.adapter.invoke('read_log', { maxLines: 1 })).log, 'last');
    }
    f.state.override = c => c.endpoint === 'GetCode' ? new Response('ERROR: legacy failure', { headers: { 'content-type': 'text/plain' } }) : undefined;
    assert.equal((await f.adapter.invoke('read_log', {})).log, 'ERROR: legacy failure');
...
test('checkouts and history preserve XML values and reject malformed or failed responses', async () => {
    const f = fixture('read-only');
    f.pending.add('script-id');
    const result = await f.adapter.invoke('list_checked_out_items', { includeAllUsers: true });
    assert.equal(result.items[0].name, 'A & B');
    assert.equal(result.items[0].language, 'CHS');
...
test('execution uses RunScript parameters and output limits without saving data sources', async () => {
    const f = fixture();
    const script = await f.adapter.invoke('execute_server_script', { uri: scriptUri, parameters: ['long output'], entryPoint: 'Run', maxCharacters: 5 });
    assert.equal(script.outputEncoding, 'text-fragment');
    assert.equal(script.output.length, 5);
...
test('table edits require checkout, stable version and target identity, then verify semantic readback', async () => {
    const f = fixture();
    const read = await f.adapter.invoke('get_table_definition', { uri: tableUri });
    ...
    const saved = await f.adapter.invoke('edit_table', edit);
    assert.equal(saved.verified, true);
    assert.match(f.state.definition, /FieldCaptionDTO/);
    ...
    assert.equal((await f.adapter.invoke('checkin_table', { uri: tableUri, reason: 'test' })).verified, true);
    f.state.definition = definition;
    f.pending.add('table-id');
    f.state.staleOnStatus = true;
...
test('standalone menus plan without writes, verify creation, and reject replay hazards', async () => {
    const ro = fixture('read-only');
    const readPlan = await ro.adapter.invoke('plan_menu_item', menuInput);
    assert.equal(readPlan.resolvedRoles[0].ROLEID, 'role-id');
    assert.equal(ro.state.menuWrites, 0);
    await assert.rejects(ro.adapter.invoke('apply_menu_item', { planId: readPlan.planId }), /read-only/);
    const f = fixture();
    const plan = await f.adapter.invoke('plan_menu_item', menuInput);
    ...
    const replay = await f.adapter.invoke('apply_menu_item', { planId: plan.planId });
    assert.equal(replay.configurationVerified, true);
    assert.equal(replay.runtimeVerified, false);
    assert.equal(f.state.menuWrites, 1);
    const stale = fixture();
    const p = await stale.adapter.invoke('plan_menu_item', menuInput);
    ...
    const p2 = await failed.adapter.invoke('plan_menu_item', menuInput);
    failed.state.failMenu = true;
...
for (const policy of ['read-only', 'allow-writes']) {
    test(`external HTTP MCP client discovers and invokes standalone ${policy} tools`, async (t) => {
        ...
        const client = new Client({ name: 'external-insight', version: 'test' });
        t.after(() => client.close());
        await client.connect(new StreamableHTTPClientTransport(new URL(handle.url)));
        const names = (await client.listTools()).tools.map(t => t.name);
        assert.equal(names.length, policy === 'read-only' ? 14 : 29);
        assert.ok(names.includes('read_log'));
        assert.ok(names.includes('get_menu_configuration'));
        ...
        assert.equal(result.structuredContent.log, 'error');
        if (policy === 'allow-writes') {
            ...
            assert.deepEqual(executed.structuredContent.output, [42]);
        }
    });
}
//# sourceMappingURL=standalone-server-tools.test.js.map