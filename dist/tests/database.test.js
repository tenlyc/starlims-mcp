...
test('restricted query validation rejects side effects and injection before dispatch', () => {
    for (const sql of ['SELECT ORDNO FROM ORDERS WHERE ORDNO = ?', 'SELECT COUNT(*) AS N FROM ORDERS WHERE ORDNO = ?', 'SELECT COALESCE(ORDNO, ?) AS N FROM ORDERS'])
        assert.equal(prepareDatabaseQuery({ sql, parameters: ['O1'] }).maxRows, 100);
    for (const sql of ['DELETE FROM ORDERS', 'SELECT * INTO BACKUP FROM ORDERS', 'SELECT 1; DELETE FROM ORDERS', 'SELECT dbo.fn(?)', 'SELECT * FROM OPENQUERY(x,?)', 'SELECT NEXT VALUE FOR SEQ', 'SELECT * FROM a.b.c', 'SELECT 1 --comment', 'SELECT /*hint*/ 1', "SELECT 'value'", 'SELECT * FROM ORDERS WITH (UPDLOCK)', 'WITH x AS (SELECT 1 AS N) SELECT * FROM x'])
        assert.throws(() => prepareDatabaseQuery({ sql, parameters: [] }));
...
test('changes require parameterized simple DML and bounded limits', () => {
    ...
    for (const sql of ['UPDATE T SET X = ?', 'DELETE FROM T', 'TRUNCATE TABLE T', 'UPDATE T SET X = 1 WHERE ID = ?', 'UPDATE T SET X = ? WHERE ID = ? OR 1=1', 'INSERT INTO T SELECT * FROM X', 'DELETE FROM T WHERE ID = ?; DELETE FROM X'])
        assert.throws(() => prepareDatabaseChange({ ...change, sql }));
...
test('server validation lists and transaction ordering remain aligned', () => {
    ...
    assert.ok(ssl.includes(QUERY_FORBIDDEN.join('|')));
    assert.ok(ssl.includes(QUERY_CALLS.join('|')));
...
test('standalone MCP demands a fresh human confirmation for every write', async (t) => {
    ...
    assert.equal((await client.callTool({ name: 'execute_database_change', arguments: change })).isError, true);
